...
class TransformInfo {
  ...
  TransformInfo({this.index, this.position, this.width, this.height, this.activeIndex, this.fromIndex, this.forward, this.done, this.viewportFraction, this.scrollDirection});
}
...
class TransformerPageController extends PageController {
  ...
  TransformerPageController({
    ...
    this.itemCount,
    this.reverse: false,
  }) : super(initialPage: TransformerPageController._getRealIndexFromRenderIndex(initialPage ?? 0, loop, itemCount, reverse)!, keepPage: keepPage, viewportFraction: viewportFraction);
...
class TransformerPageView extends StatefulWidget {
  ...
  static PageController createPageController({required bool reverse, int? index, int? itemCount, required bool loop, required double viewportFraction}) {
    return new PageController(initialPage: getRealIndexFromRenderIndex(reverse: reverse, index: index, itemCount: itemCount, loop: loop)!, viewportFraction: viewportFraction);
  }
}
...
class _TransformerPageViewState extends State<TransformerPageView> {
  ...
  void initState() {
    ...
    if (_pageController == null) {
      _pageController = new TransformerPageController(initialPage: widget.index, itemCount: widget.itemCount, loop: widget.loop, reverse: widget.transformer == null ? false : widget.transformer!.reverse);
    }
